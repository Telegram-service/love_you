...
async def load_reload(event):
    ...
    try:
        ...
        msg = await event.respond(f"Successfully (re)loda plugin {shortname}")
...
@borg.on(util.admin_cmd(pattern="sund plugin (?P<shortname>\w+)$"))  # pylint:disable=E0602
async def send_plug_in(event):
    ...
    await event.edit("plugin uploaded successfully {} in {} seconds".format(input_str, time_taken_in_ms))
    await asyncio.sleep(DELETE_TIMEOUT)
    await event.delete()
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit("opps! Can't istall this plugin.")
        except Exception as e:  # pylint:disable=C0103,W0703
