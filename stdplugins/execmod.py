...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's SUICIDE BOMB:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's PIP UPGRADE:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's Date & Time Of India:**\n\n\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's Environment Module:**\n\n\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's Neofetch Installed, Use `.sysd` :**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's Telethon Updated**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Server Speed Calculated:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Coffeehouse Updated:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Bot Installed To Love_you**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Telegram Installed To Love_you**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , PIP Installed To Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Name generator for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , fast.com for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , password generator for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , fortune teller for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , dad jokes for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , quotes for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , fake id generator for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , kwot for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , programming quotes for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Simple Password Generator for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Coronavirus Stats of India for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**Surajit's , Coronavirus Stats of the World for Your Love_you...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
